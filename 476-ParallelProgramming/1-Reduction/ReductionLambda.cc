/*
  Filename   : ReductionLambda.cc
  Author     : Steve Zelek
  Course     : CSCI 476-01
  Assignment : Program 1 (EC1&2)
...
//possible binary associative operations for the reduction to use:
int
...
void allReductionStage(std::vector<int> &nums, std::function<int(int, int)> operation, unsigned stageNum){
    ...
    while (coreNum + offSet < nums.size())
    {
        for (int i = 0; i < offSet; ++i){
            int result = operation(nums[coreNum], nums[coreNum+offSet]);
            nums[coreNum] = result;
